...
function initFormHandler() {
  ...
  const tempForm = document.getElementById('new-recipe');
  ...
  tempForm.addEventListener('submit', function() {
    ...
    tempRecipe.data = recipeObject.data;
    const mainE = document.querySelector('main');
    mainE.append(tempRecipe);
    let arr = JSON.parse(localStorage.getItem('recipes'));
    arr.append(tempRecipe.data);
  ...
  const buttonDanger = document.querySelector('.danger');
  // B11. TODO - Add a click event listener to clear local storage button
  buttonDanger.addEventListener('click', function() {
    localStorage.clear();
    const clearMain = document.querySelector('main');
    clearMain.innerHTML = '';
  });
